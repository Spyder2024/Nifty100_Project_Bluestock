...
def load_clustering_features(db_path: Path = DEFAULT_DB_PATH) -> pd.DataFrame:
    ...
    for _, row in df_comps.iterrows():
        ...
        sec = str(
            row["sector_name"] if pd.notna(row["sector_name"]) else "Unclassified"
        ).strip()
...
def impute_features(
    df: pd.DataFrame, feature_cols: Sequence[str] = FEATURE_COLUMNS
) -> pd.DataFrame:
...
def plot_elbow(
    ...
    ax.set_title(
        "Nifty 100 KMeans Clustering — Elbow Curve Analysis",
        fontsize=12,
        fontweight="bold",
        pad=12,
        color="#0F172A",
    ...
    ax.set_xlabel(
        "Number of Clusters (k)", fontsize=10, fontweight="bold", color="#334155"
    )
    ax.set_ylabel(
...
def run_clustering(
    ...
    # 4. Elbow Plot (k=2..10)
    elbow_data = compute_elbow(
        X_scaled, k_range=range(2, 11), random_state=random_state
    )
    plot_elbow(elbow_data, output_path=elbow_png, chosen_k=n_clusters)

    # 5. Fit KMeans with k=5
    kmeans, labels, distances = fit_kmeans(
        X_scaled, n_clusters=n_clusters, random_state=random_state
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Run KMeans Clustering on Nifty 100 constituents."
    )
    parser.add_argument(
        "--db", type=str, default=str(DEFAULT_DB_PATH), help="Path to SQLite database."
    )
    parser.add_argument(
        "--output-csv",
        type=str,
        default=str(DEFAULT_OUTPUT_CSV),
        help="Output path for cluster CSV.",
    )
    parser.add_argument(
        "--elbow-png",
        type=str,
        default=str(DEFAULT_ELBOW_PNG),
        help="Output path for elbow plot.",
    )
    parser.add_argument(
        "--k", type=int, default=5, help="Number of clusters (default: 5)."
    )
    ...
    print("\n[SAMPLE ASSIGNMENTS]")
    sample_df = df_res[
        ["company_id", "sector", "cluster_id", "cluster_name", "distance_from_centroid"]
    ].head(12)
