...
INDEX_DEFINITIONS = [
    # 1. Ratios Table
    ("idx_ratios_company_id", "CREATE INDEX IF NOT EXISTS idx_ratios_company_id ON ratios(company_id)"),
    ("idx_ratios_year", "CREATE INDEX IF NOT EXISTS idx_ratios_year ON ratios(year)"),
    ("idx_ratios_comp_year", "CREATE INDEX IF NOT EXISTS idx_ratios_comp_year ON ratios(company_id, year)"),
    ("idx_ratios_roe", "CREATE INDEX IF NOT EXISTS idx_ratios_roe ON ratios(roe)"),

    # 2. Income Statement Table
    ("idx_is_company_id", "CREATE INDEX IF NOT EXISTS idx_is_company_id ON income_statement(company_id)"),
    ("idx_is_year", "CREATE INDEX IF NOT EXISTS idx_is_year ON income_statement(year)"),
    ("idx_is_comp_year", "CREATE INDEX IF NOT EXISTS idx_is_comp_year ON income_statement(company_id, year)"),

    # 3. Balance Sheet Table
    ("idx_bs_company_id", "CREATE INDEX IF NOT EXISTS idx_bs_company_id ON balance_sheet(company_id)"),
    ("idx_bs_year", "CREATE INDEX IF NOT EXISTS idx_bs_year ON balance_sheet(year)"),
    ("idx_bs_comp_year", "CREATE INDEX IF NOT EXISTS idx_bs_comp_year ON balance_sheet(company_id, year)"),

    # 4. Cash Flow Table
    ("idx_cf_company_id", "CREATE INDEX IF NOT EXISTS idx_cf_company_id ON cash_flow(company_id)"),
    ("idx_cf_year", "CREATE INDEX IF NOT EXISTS idx_cf_year ON cash_flow(year)"),
    ("idx_cf_comp_year", "CREATE INDEX IF NOT EXISTS idx_cf_comp_year ON cash_flow(company_id, year)"),

    # 5. Market Cap Table
    ("idx_mcap_company_id", "CREATE INDEX IF NOT EXISTS idx_mcap_company_id ON market_cap(company_id)"),
    ("idx_mcap_year", "CREATE INDEX IF NOT EXISTS idx_mcap_year ON market_cap(year)"),
    ("idx_mcap_comp_year", "CREATE INDEX IF NOT EXISTS idx_mcap_comp_year ON market_cap(company_id, year)"),

    # 6. Prices Table
    ("idx_prices_company_id", "CREATE INDEX IF NOT EXISTS idx_prices_company_id ON prices(company_id)"),
    ("idx_prices_year", "CREATE INDEX IF NOT EXISTS idx_prices_year ON prices(year)"),
    ("idx_prices_comp_year", "CREATE INDEX IF NOT EXISTS idx_prices_comp_year ON prices(company_id, year)"),

    # 7. Companies Table
    ("idx_companies_sector_id", "CREATE INDEX IF NOT EXISTS idx_companies_sector_id ON companies(sector_id)"),
]
