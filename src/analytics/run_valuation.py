...
from src.analytics.valuation import compute_all_valuations, load_valuations
...
def main() -> None:
    ...
    parser.add_argument(
        "--wacc", type=float, default=0.12,
        help="WACC (default: 0.12).",
    )
    parser.add_argument(
        "--terminal-growth", type=float, default=0.05,
    ...
    print(f"Computing valuations  WACC={args.wacc}  "
    ...
    if not val_df.empty:
        ...
        for col in ("graham_number", "dcf_intrinsic_value",
                     "ddm_intrinsic_value", "relative_avg_value"):
