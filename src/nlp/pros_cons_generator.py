...
def _conf_sustained(
    ...
    if threshold > 0:
        signal = (
            max(0, (latest_val - threshold) / threshold)
            if above
...
def _sustained(
    df: pd.DataFrame, col: str, threshold: float, min_yrs: int, above: bool = True
...
def _trend(
    df: pd.DataFrame, col: str, min_yrs: int, direction: str
) -> Optional[tuple[int, float]]:
...
def _fallback_pro(cm: dict, d: pd.DataFrame, company_id: str) -> tuple:
    ...
    return (
        ...
        "market presence and listing credentials",
        61,
    )
...
def _fallback_con(cm: dict, d: pd.DataFrame) -> tuple:
    ...
    return (
        "CON_FB",
        "con",
        "Requires ongoing monitoring of competitive position " "and sector dynamics",
        61,
    )
