...
def test_profile_clusters():
    ...
    profiles = profile_clusters(
        df_clustered, feature_cols=["return_on_equity_pct", "debt_to_equity"]
    )
    ...
    assert (
        profiles.loc[profiles["cluster_id"] == 0, "return_on_equity_pct_mean"].iloc[0]
        == 15.0
    )
...
def test_generate_correlation_heatmap(sample_kpis_df, tmp_path):
    ...
    result = generate_correlation_heatmap(
        sample_kpis_df, output_path=out_png, kpi_cols=KPI_10_COLUMNS
    )
...
def test_detect_sector_outliers(sample_kpis_df):
    ...
    df_outliers = detect_sector_outliers(
        sample_kpis_df, metric_cols=KPI_10_COLUMNS, threshold=1.8
    )
